            ...
            async for content in self.tokenizer_manager.generate_request(
                ...
                if finish_reason_type:
                    ...
                    if finish_reason_type == "abort" and isinstance(
                        ...
                        # Nothing has streamed yet, so HTTP 200 has not been
                        # committed -- raise so the caller can return the real
                        # HTTP status (429 queue-full, 400 scheduler rejects
                        # such as unsupported grammar under spec decoding, 503
                        # waiting timeout) instead of an in-band SSE frame that
                        # OpenAI-dialect proxies misread as a provider failure.
                        # Mid-stream aborts must still use the frame; the
